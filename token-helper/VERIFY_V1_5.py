...
def run():
    ...
    c('bootstrap_marker','bootstrap-v4-pre-init-location-subscribe' in html)
    ...
    c('location_handler_unique',html.count("e.detail.type==='VKWebAppLocationChanged'")==1)
    c('location_subscription_function','function installLocationSubscription()' in html)
    c('location_subscription_called','installLocationSubscription();' in html)
    c('location_pre_init_stage','VK_LOCATION_SUBSCRIBED_PRE_INIT' in html)
    c('location_event_stage','VK_LOCATION_EVENT' in html)
    c('location_source_pre_init','LOCATION_CHANGED_PRE_INIT' in html)
    call=html.index('installLocationSubscription();')
    init=html.index("INIT=withTimeout(bridge.send('VKWebAppInit',{})")
    c('subscribe_call_before_init',call < init)
    c('old_post_init_source_removed',"maybeAutoConnect((e.detail.data||{}).location||'','LOCATION_CHANGED')" not in html)
